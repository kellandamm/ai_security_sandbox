...
3. Authorize the Azure OpenAI call through OPA.
4. Call the model using managed identity.
5. Parse tool calls.
6. Enforce the Python capability manifest.
7. Authorize the specific action through OPA.
8. Request human approval for high-risk actions.
9. Execute the tool in the sandbox.
10. Emit audit events for policy decisions, tool results, file hashes, destinations, token counts, and outcomes.
