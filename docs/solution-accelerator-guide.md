...
The accelerator is intentionally opinionated. It favors fail-closed behavior, managed identity, and per-agent capability isolation over a broad general-purpose agent framework.

## Environment Model

This repository contains the AI Security Sandbox application and supporting infrastructure only.

- Frontend and API are deployed as standalone services in your Azure environment.
- Microsoft Entra ID is used for authentication.
- API Management validates bearer tokens and applies edge controls before forwarding to the orchestrator.
- Multi-tenant sign-in can be retained via the `organizations` endpoint when required by your production identity design.
...
The frontend includes attack and workspace templates for repeatable security validation:
...
The workspace test documents live in the repository under [../frontend/public/templates/workspace-tests](../frontend/public/templates/workspace-tests).

| Test document | Repository path | Scenario |
|---|---|---|
| Workspace test README | `frontend/public/templates/workspace-tests/README.md` | Upload pack overview and recommended execution order. |
| Deny tool not allowed | `frontend/public/templates/workspace-tests/deny-tool-not-allowed.md` | Capability allowlist and OPA deny validation. |
| Kill switch validation | `frontend/public/templates/workspace-tests/kill-switch-validation.md` | App Configuration kill-switch containment and recovery. |
| OPA unreachable simulation | `frontend/public/templates/workspace-tests/opa-unreachable-simulation.md` | Fail-closed behavior when the policy engine is unavailable. |
| Approval high-risk HTTP POST | `frontend/public/templates/workspace-tests/approval-high-risk-http-post.md` | Human approval flow validation with temporary policy setup. |

The attack-library templates are available under [../frontend/public/templates](../frontend/public/templates):

| Template | Repository path |
|---|---|
| Prompt injection CSV | `frontend/public/templates/prompt-injection.csv` |
| Path traversal task | `frontend/public/templates/path-traversal-task.txt` |
| Credential harvest JSON | `frontend/public/templates/credential-harvest.json` |
| Token bomb text | `frontend/public/templates/token-bomb.txt` |
...
5. Walk through the portal demo motion in [portal-demo-talk-track.md](portal-demo-talk-track.md) with security and platform stakeholders.
